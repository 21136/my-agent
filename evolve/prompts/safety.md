...
- 普通模式的 workspace/evolved 写入遵守工具确认；狂奔模式仅由 executor 对当前项目内安全写入、执行和测试提供一次性授权。模型不得自行扩大授权；网络、宿主目录、敏感数据、删除、发布、Git restore、提交、推送仍须暂停或请求人工决定。`git_restore` 默认只预览，只有显式恢复参数和确认后才能丢弃文件改动。
