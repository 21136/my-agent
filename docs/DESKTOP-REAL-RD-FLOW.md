...
```text
REQ-*  AC-*  UX-*  TD-*  ADR-*  T-*  V-*  REL-*  CHG-*  IT-*  S-*
```
...
### 7.3 文档基线补齐 UX（v1 · T-5831）

第一版 **不新增** 专门的「生成文档基线」按钮，避免再引入一条容易卡住的 UI 通路。补齐路径固定为：

1. 侧栏显示 **「文档基线不完整」** 与具体缺项；
2. Plan Agent 生成文档补齐提案；
3. 用户走现有 **审阅 → 采纳** 链路写入。

后续若提案链稳定，可把同一流程做成快捷入口，但仍必须走 **提案 → 审阅 → 采纳**，**禁止**直接写盘。
