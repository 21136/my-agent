...
> **M5**：主栏不再渲染整份 TASKS 长卷。**现行真源（目标）** = [§6.2.2 单项目态势侧栏（UX-026）](#622-单项目态势侧栏ux-026--已决--待实施--2026-08-05)。  
> 下方 §6.1 ASCII 为 **历史示意**（完整任务流长卷，已废止为侧栏主视图）。
...
### 6.2.1 底栏脚注（UX-024 · 2026-08-05）

> **问题**：壳合并后保留的 `#project-plan-card`（compat）在 `plan_status=draft` 时仍被展示，插在图标栏**下方**，把底栏整体顶上去。  
> **决议**：计划确认 UI 只保留 **header meta** + **`sidebar-change-banner`**（含「确认开工」）；compat 节点 **永不 `display`**，仅保留按钮 id 供 `index.ts` 事件绑定。
...
.unified-sidebar
  ...
  .sidebar-services
  .sidebar-task-flow          ← flex:1，中间区滚动
  .sidebar-footer             ← margin-top:auto，钉视口底
    .sidebar-change-banner    ← 待确认 / 变更 / 建议卡（有则显）
    .sidebar-icon-bar         ← ◎ ☰ 📄 ✓ …
  #project-plan-card.hidden   ← compat，不展示
  .sidebar-overlay            ← absolute 覆盖
```

验收见 [UX-POLISH.md](./UX-POLISH.md) **S-UX-024**。

### 6.2.2 单项目态势侧栏（UX-026 · 已决 · 待实施 · 2026-08-05）

> **动机**：侧栏应回答「当前这场战役的态势」，不是「多项目目录」也不是「agent 工具终端」。  
> **非目标**：OpenCode 式侧栏项目树；Faber 式全屏 Kanban 替代主聊；在侧栏展开 `read_file` / `glob_file_search` 明细列表。  
> **关联**：[UX-POLISH.md](./UX-POLISH.md) UX-026 · UX-024（footer 钉底）· [DESKTOP.md](./DESKTOP.md)（主聊过程块为执行真源）。

#### 6.2.2.1 原则

| ID | 决议 |
|----|------|
| **SP-1** | **单项目专注**：侧栏只展示 **当前 `project_id` 绑定** 的态势；不在侧栏挂多项目树 |
| **SP-2** | **多项目切换**：顶栏项目名 `▾`（最近项）+ 底栏 **▢** overlay「我的项目」；选中后 **整栏切换**，不并排保留上一项目内容 |
| **SP-3** | **进度一种说法**：侧栏头仅 **「还有 N 条开放」** + 细进度条；禁止同屏 `N/M 未完成` 与 `X/M 完成 · N 开放` 双轨 |
| **SP-4** | **当前任务为 C 位**：侧栏主体一块 **当前** 卡（任务 id + 2～3 行摘要）；任务正文 **不** 在本回合区重复 |
| **SP-5** | **本回合一行摘要**：`▸ 本回合 · K 工具 · …`；点击 **跳转主聊** 并展开对应过程块；侧栏 **不** 列工具行 |
| **SP-6** | **Services 默认折叠**：`▸ 服务 · N 个已停止`；展开后列表 `max-height` 受限、内滚 |
| **SP-7** | **待拍板类型单槽**：undo / 搭档忙 / **待采纳** / 计划待确认 等 **不同类型** banner — 同时只露 **优先级最高一类**（沿用 `renderProjectSidebar` banner 链）；**不**与 SP-9 混淆 |
| **SP-8** | **执行日志归主聊**：工具明细、思考、失败 tail 仅在主聊 `unified-process`；侧栏不重复 |
| **SP-9** | **待采纳多卡堆叠**：`N ≥ 1` 时标题 **「待采纳 · N」**；**仅顶卡**完整展示（标题 + 摘要 + `+n −m` + 查看/采纳/忽略）；其后 1～2 张以 **层叠露出下缘/侧缘**（视觉深度，非平铺长列表）；顶卡采纳或忽略后 **下一张升为顶卡**（无 ‹ › 翻页）；堆叠区 **固定高度预算**（约 1 张完整卡 + 堆叠 peek），不把「当前」顶出视口；**落点 = 侧栏体**，在 **当前卡之下、本回合摘要之上**（**不**进脚区）；可选 **「展开全部」** 在堆叠区内滚看全文（M1） |
| **SP-10** | **采纳反馈双态**：（1）**堆叠位闪绿** — 用户 **采纳** 顶卡且 **仍有下一张** 时，SP-9 堆叠槽 **~1.5s** 变为绿色「已采纳写入 …」态（一行、**无**按钮、无 diff），再自动切下一张顶卡；此期间 **不出**脚区绿条。（2）**脚区反馈条** — 队列 **已空** 后最后一次采纳（或仅有告知、无堆叠时）：`.sidebar-footer` 图标栏上常驻绿条 +「关闭」（[PLAN-REVIEW-UI](./PLAN-REVIEW-UI.md) A3）。**忽略**顶卡：堆叠槽 **不**绿闪，直接切下一张（可选 ~0.5s 淡出）。DOM：堆叠闪绿 `.sidebar-suggestion-stack.is-adopt-flash`；脚区 `.sidebar-adopted-banner` |

#### 6.2.2.2 布局（常态 · 宽约 260px）

三段式：**头（固定）→ 体（滚动）→ 脚（钉底，`sidebar-footer`）**。

```
┌─ 侧栏 · 仅当前项目 ─────────────────┐
│  huiyi                          ▾   │  ← SP-1/2：项目名 + 切换，非列表
│  还有 12 条开放                      │  ← SP-3
│  ▓▓▓▓▓▓▓▓░░░░░░░░░░░░░░░░░░░░░░░░  │
│─────────────────────────────────────│
│ ┌─ 当前 ──────────────────────────┐ │
...
│ │ 仪表盘首页：登录后默认展示概览页  │ │
│ │ …（最多 3 行，超出 …）            │ │
│ │ [ 查看完整计划 ]                 │ │  → overlayPanel=plan
│ └─────────────────────────────────┘ │
│ ┌─ 待采纳 · 3 ────────────────────┐ │  ← SP-9 · **体**内堆叠
│ │ [顶卡] … [ 查看 ] [ 采纳 ] [ 忽略 ]│ │
│ │   ╲╲ peek                         │ │
│ └─────────────────────────────────┘ │
│   ↓ 采纳且仍有下一张（SP-10 闪绿 ~1.5s）│
│ ┌─ 已采纳写入 ────────────────────┐ │  ← 同槽位临时绿态，无按钮
│ │ 已采纳写入 TASKS.md              │ │
│ └─────────────────────────────────┘ │
│   ↓ 自动切下一张顶卡                  │
...
| 区块 | DOM / 落点（目标） | 行为 |
|------|-------------------|------|
| **头** | `.unified-sidebar-header` | 项目名、`▾` 最近项目（M1）；开放数 + 细条；**不**写「计划待确认」（交给待拍板槽或顶栏） |
| **当前卡** | `.sidebar-task-flow` / `.sidebar-decision-current` | 首条开放任务或 `turnArmedText`；无任务时「暂无开放任务」+ 查看完整计划 |
| **待拍板** | `.sidebar-footer .sidebar-change-banner` | SP-7 类型单槽：undo / 搭档忙 / **已采纳** / 外部修改 / 计划确认 …（**不含**待采纳堆叠） |
| **待采纳堆叠** | `.sidebar-task-flow` 内 **新** `.sidebar-suggestion-stack` | SP-9：体 · 当前下；顶卡可操作；`N>1` peek；**从脚区迁出**（现状误放在 `#sidebar-change-banner`） |
| **已采纳反馈** | `.sidebar-suggestion-stack.is-adopt-flash`（体）· `.sidebar-adopted-banner`（脚） | SP-10：中间队列 → 堆叠位 ~1.5s 绿闪再切顶卡；末条 / 无堆叠 → 脚区绿条 + 关闭 |
| **本回合摘要** | **新** `.sidebar-turn-summary`（待建） | 数据来自 `turnEvidence` / 本回合 process；默认折叠一行；**移除**侧栏 `sidebar-turn-evidence` 明细列表 |
| **服务** | `.sidebar-services` | 默认 `collapsed`；标题行可点展开；保留刷新/日志 |
| **脚** | `.sidebar-footer` | `change-banner`（若有）+ `sidebar-icon-bar`；**不**展示 `#project-plan-card`（UX-024） |

#### 6.2.2.4 底栏图标（本项目内，语义不变）

| 图标 | panel | 说明 |
|------|-------|------|
| ◎ | `tasks` / 当下 | 侧栏回到 §6.2.2.2 态势视图 |
| ☰ | `plan` | 完整 TASKS overlay |
| 📄 | `docs` | MAP / 文档 overlay |
| ✓ | `verify` | 验收（`plan_status=confirmed` 后显示） |
| ⎇ | `threads` | 本会话线 overlay |
| ▢ | `projects` | **我的项目**列表 overlay（**唯一**多项目列表入口） |

**废止**常驻文案：「改计划会在主输入自动交给计划搭档…」→ 首次 ☰ 或帮助页 tooltip（M1 可选）。
...
- 待拍板槽显示「计划待确认」+ 按钮；无 TASKS 时副文案「等待生成 TASKS.md」
- **不**在图标栏下显示 compat `#project-plan-card`

**C · 助手执行中**

- 当前卡左边线轻 pulse（可选，沿用 `is-working` 色系）
- 本回合摘要：`▾ 本回合 · 进行中 · 已 K 工具…`；仍不展开工具列表
...
| 阶段 | 内容 | 档位 |
|------|------|------|
| **M0** | 去掉侧栏本回合证据长列表 → 一行摘要 + 跳主聊；进度文案单轨；Services 默认折叠；删常驻 plan-channel hint；**待采纳堆叠迁到体**（SP-9）；**采纳堆叠位闪绿 + 脚区末条反馈**（SP-10） | P1 |
| **M1** | 顶栏/侧栏头 `▾` 最近项目；待拍板与当前卡视觉层级微调；堆叠区 **「展开全部 N 条」** 内滚（可选） | P2 |
| **M2** | 本回合摘要执行中 pulse；顶栏与侧栏头字段去重 | P2 |
