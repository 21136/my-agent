...
按任务标题/标签启发式归类（规则优先；缺省偏严）：

| 证据类 | 典型标题信号 | 接受的成功证据（示例） |
|--------|--------------|------------------------|
| **write** | Entity / Mapper / Service / Controller / 页面 / `.vue` / `.java` | 本回合对该任务相关路径的 `write_text` / `patch_file` 等 **ok** |
...
| **unknown** | 无法归类 | **不自动勾** → 异常卡（人审规则 / 改任务文案） |

> 实施可将信号做成可测的纯函数 + 单测表；允许后续加 `TASKS` 行内标签如 `[evidence:compile]`（非本版必做）。
